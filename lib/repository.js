...
/** Nhập nhiều dòng trong một transaction; tự tạo dự án chưa tồn tại theo tên. */
export function importItems(rows) {
  const db = getDb();
  const projectsByName = new Map(
    db.prepare("SELECT id, name FROM projects").all().map((project) => [project.name.trim().toLocaleLowerCase("vi"), project.id])
  );
  ...
  try {
    for (const row of rows) {
      const projectKey = row.project.name.trim().toLocaleLowerCase("vi");
      let projectId = projectsByName.get(projectKey);
      if (!projectId) {
        projectId = randomUUID();
        const ts = nowIso();
        insertProject.run(
          projectId, row.project.name, row.project.owner, row.project.bid,
          row.project.location, ts, ts
        );
        projectsByName.set(projectKey, projectId);
        projectsCreated += 1;
      }

      const ts = nowIso();
      insertItem.run(
        randomUUID(), projectId, row.item.orderNo, row.item.name, row.item.unit,
        row.item.planQty, row.item.receivedQty, row.item.receivedDate,
        row.item.model, row.item.maker, row.item.note, ts, ts
      );
      if (row.project.owner) importedOwners.add(row.project.owner);
  ...
  return { imported: rows.length, projectsCreated };
}

function mergeSettingValues(db, key, additions) {
