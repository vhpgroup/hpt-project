...
export const BID_TYPES = ["Thầu giấy", "Thầu trực tuyến"];
...
export function validateProject(input, { partial = false } = {}) {
  ...
  if (!partial || input.bidDate !== undefined) {
    const bidDate = trim(input.bidDate) || null;
    if (bidDate && !DATE_RE.test(bidDate)) {
      errors.bidDate = "Ngày nộp thầu phải theo định dạng YYYY-MM-DD.";
    } else out.bidDate = bidDate;
  }
  if (!partial || input.bidType !== undefined) {
    const bidType = trim(input.bidType) || "";
    if (bidType && !BID_TYPES.includes(bidType)) {
      errors.bidType = "Loại thầu phải là Thầu giấy hoặc Thầu trực tuyến.";
    } else out.bidType = bidType;
  }
