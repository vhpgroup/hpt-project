...
export function createItem(data) {
  ...
  getDb()
    .prepare(
      `INSERT INTO items (id, package_id, order_no, name, unit, plan_qty, model, maker, note, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    )
    .run(
      id, data.packageId, data.orderNo ?? "", data.name, data.unit ?? "",
      data.planQty ?? 0, data.model ?? "", data.maker ?? "", data.note ?? "", ts, ts
    );
  return getItem(id);
}
...
export function updateItem(id, patch) {
  ...
  const columns = {
    ...
    planQty: "plan_qty", model: "model", maker: "maker", note: "note",
  };
  const sets = [];
  const params = [];
  ...
  if (sets.length) {
    ...
    getDb().prepare(`UPDATE items SET ${sets.join(", ")} WHERE id = ?`).run(...params);
  }
  return getItem(id);
}
...
export function createReceipt(itemId, data) {
  ...
  const id = randomUUID();
  getDb()
    .prepare("INSERT INTO receipts (id, item_id, qty, received_date, note, created_at) VALUES (?, ?, ?, ?, ?, ?)")
    .run(id, itemId, data.qty, data.receivedDate ?? null, data.note ?? "", nowIso());
  return { receipt: mapReceipt(getDb().prepare("SELECT * FROM receipts WHERE id = ?").get(id)), item: getItem(itemId) };
}
...
export function deleteReceipt(receiptId) {
  const db = getDb();
  const existing = db.prepare("SELECT item_id FROM receipts WHERE id = ?").get(receiptId);
  if (!existing) throw new ApiError(404, "Không tìm thấy đợt nhập.");
...
export function createPackage(data) {
  ...
  try {
    getDb()
      .prepare(
        `INSERT INTO packages (id, project_id, code, name, owner, location, deadline, note, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
      )
      .run(id, data.projectId, data.code ?? "", data.name ?? "", data.owner ?? "",
           data.location ?? "", data.deadline ?? null, data.note ?? "", ts, ts);
  } catch (err) {
...
export function updatePackage(id, patch) {
  ...
  const columns = {
    ...
    location: "location", deadline: "deadline", note: "note",
  };
  const sets = [];
  const params = [];
  for (const [key, column] of Object.entries(columns)) {
...
export function deletePackage(id) {
  ...
  if (info.changes === 0) throw new ApiError(404, "Không tìm thấy gói thầu.");
}
...
export function createProject(data) {
  ...
  try {
    getDb()
      .prepare("INSERT INTO projects (id, name, location, note, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)")
      .run(id, data.name, data.location ?? "", data.note ?? "", ts, ts);
  } catch (err) {
...
export function updateProject(id, patch) {
  getProject(id);
  const columns = { name: "name", location: "location", note: "note" };
  const sets = [];
  const params = [];
  for (const [key, column] of Object.entries(columns)) {
    if (patch[key] === undefined) continue;
    ...
    for (const row of rows) {
      ...
      if (!packageId) {
        ...
        insPackage.run(
          ...
          row.package.location || row.project.location, row.package.deadline ?? null, now, now
        );
      ...
      if (existingItems.has(itemKey)) {
        ...
        db.prepare(
          "UPDATE items SET unit = ?, plan_qty = ?, model = ?, maker = ?, note = ?, updated_at = ? WHERE id = ?"
        ).run(row.item.unit, row.item.planQty, row.item.model, row.item.maker, row.item.note, now, existing.id);
        result.replaced += 1;
        continue;
      }
      ...
      insItem.run(
        ...
        row.item.planQty, row.item.model, row.item.maker, row.item.note, now, now
      );
