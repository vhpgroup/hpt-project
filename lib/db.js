...
function seedIfEmpty(conn) {
  ...
  try {
    ...
    for (const pk of seedPackages) {
      insPackage.run(pk.id, pk.projectId, pk.code, pk.name, pk.owner, pk.location, pk.deadline ?? null, now, now);
    }
    for (const i of seedItems) {
      insItem.run(i.id, i.packageId, i.orderNo, i.name, i.unit, i.planQty, i.model, i.maker, now, now);
